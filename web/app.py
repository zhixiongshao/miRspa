...
def _parse_dotbracket_pairs(structure: str) -> dict[int, int]:
    stack: list[int] = []
    pairs: dict[int, int] = {}
...
def render_hairpin_svg(
    *,
    ...
    Output is an SVG string (safe to embed).
    """
    seq = "".join(str(seq).split()).upper().replace("T", "U")
    structure = "".join(str(structure).split())
    n = min(len(seq), len(structure))
    seq = seq[:n]
    structure = structure[:n]

    pairs = _parse_dotbracket_pairs(structure)
    # Collect unique pairs (i<j)
    pair_list = sorted({(i, j) for i, j in pairs.items() if i < j})

    # Find loop region: between last paired i and j in stem (approx)
    if pair_list:
        left_max = max(i for i, _j in pair_list)
        right_min = min(j for _i, j in pair_list)
        loop_indices = [k for k in range(left_max + 1, right_min) if k not in pairs]
    else:
        loop_indices = list(range(n))

    # Layout parameters (base coordinates; will be auto-fit later)
    # Keep layout spacing stable; adjust draw radius separately to avoid overlap.
    # Layout spacing controls (coordinates before rotation / fit)
    r_layout = 12
    cx = width * 0.50
    top = 88
    # Increase spacing so circles/text can be larger without overlap.
    dy = max(42, int(r_layout * 3.2))  # step between stem rungs
    stem_gap = 220  # wider stem so paired bases separate more after rotation
    left_x = cx - stem_gap / 2
    right_x = cx + stem_gap / 2

    coords: dict[int, tuple[float, float]] = {}
    # Place paired bases as ladder
    for t, (i, j) in enumerate(pair_list):
        y = top + t * dy
        coords[i] = (left_x, y)
        coords[j] = (right_x, y)

    # Place loop on a semicircle above the top of stem
    if loop_indices:
        import math
        ...
        loop_cy = top - 10
        m = len(loop_indices)
        for u, k in enumerate(loop_indices):
            ang = math.pi * (u / max(1, m - 1))  # 0..pi
            x = loop_cx + loop_r * math.cos(math.pi - ang)
            y = loop_cy - loop_r * math.sin(math.pi - ang)
            coords[k] = (x, y)

    # Place remaining unpaired bases (bulges) near nearest paired position
    for k in range(n):
        if k in coords:
            continue
        # find closest paired index
        nearest = None
        best = 10**9
        for i, j in pair_list:
            for idx in (i, j):
                ...
                if d < best:
                    best = d
                    nearest = idx
        if nearest is None or nearest not in coords:
            coords[k] = (cx, top + (k * dy))
        else:
            x0, y0 = coords[nearest]
            # offset slightly outward
            x = x0 + (-36 if x0 < cx else 36)
            y = y0 + (best * 0.25)
            coords[k] = (x, y)

    # Rotate layout if desired (set to 0° for vertical).
    if coords:
        xs0 = [xy[0] for xy in coords.values()]
        ys0 = [xy[1] for xy in coords.values()]
        cx0 = (min(xs0) + max(xs0)) / 2.0
        cy0 = (min(ys0) + max(ys0)) / 2.0

        def R(x: float, y: float) -> tuple[float, float]:
            # rotation around (cx0, cy0)
            # [x']   [ cos  sin][x]
            # [y'] = [-sin  cos][y]
            dx = x - cx0
            dy = y - cy0
            ...
            rx = dx * ca + dy * sa
            ry = -dx * sa + dy * ca
            return (cx0 + rx, cy0 + ry)

        coords = {k: R(x, y) for k, (x, y) in coords.items()}

    # Auto-fit:
    # - Always fit by width (so circles can stay large).
    # - Let height grow with content; the frontend container can scroll.
    xs = [xy[0] for xy in coords.values()]
    ys = [xy[1] for xy in coords.values()]
    min_x, max_x = min(xs), max(xs)
    min_y, max_y = min(ys), max(ys)

    # Padding controls how much whitespace we keep around the drawing.
    # Keep it minimal so the diagram fills the canvas.
    pad0 = 0.0

    def fit_with_pad(pad: float) -> tuple[float, float, float, float]:
        bw = max(1.0, (max_x - min_x) + 2 * pad)
        bh = max(1.0, (max_y - min_y) + 2 * pad)
        avail_w = max(1.0, width - 2 * pad)
        scale = avail_w / bw
        # Center horizontally; align to top vertically (then height can extend downward).
        tx = (width / 2.0) - scale * ((min_x + max_x) / 2.0)
        ty = pad - scale * min_y
        out_h = max(float(height), (scale * bh) + 2 * pad)
        return scale, tx, ty, out_h

    # First fit (base padding), then estimate safe draw radius,
    # then refit with a padding that accounts for circle radius to avoid clipping.
    scale, tx, ty, out_h = fit_with_pad(pad0)

    def T(x: float, y: float) -> tuple[float, float]:
        return (x * scale + tx, y * scale + ty)

    # Pick a draw radius that won't overlap.
    # We estimate the minimal neighbor distance in the final (T-transformed) coordinates.
    # Using local neighbors (adjacent bases + paired base) is more stable than O(n^2) global min,
    # and avoids r_draw collapsing to ~0 due to rare near-overlaps elsewhere.
    import math
    pts: list[tuple[float, float]] = [T(*coords[k]) for k in range(n)]
    min_d = float("inf")
    for k in range(n):
        x1, y1 = pts[k]
        cand: list[int] = []
        ...
        for j in cand:
            ...
            if 0.0 < d < min_d:
                min_d = d
    # Keep circles readable, but never overlapping.
    # Allow for stroke width and a tiny safety gap.
    r_cap = 17.2  # ~2x previous cap (8.6)
    # Keep a small visible minimum; still bounded by min_d so we avoid overlaps.
    r_min = 3.5
    r_draw = r_cap
    if min_d != float("inf"):
        r_draw = min(r_cap, max(r_min, (min_d * 0.5) - 1.4))

    # Refit with a small radius-aware padding to prevent clipping at edges,
    # but keep whitespace minimal.
    pad1 = max(pad0, r_draw + 2.0)
    scale, tx, ty, out_h = fit_with_pad(pad1)
    pts = [T(*coords[k]) for k in range(n)]
    min_d = float("inf")
    for k in range(n):
        x1, y1 = pts[k]
        cand: list[int] = []
    ...
    font_px = max(10.0, min(18.0, r_draw * 1.20))

    # SVG render
    parts: list[str] = []
    out_h_i = int(max(1.0, round(out_h)))
    parts.append(
        f'<svg xmlns="http://www.w3.org/2000/svg" width="100%" height="{out_h_i}" '
        f'viewBox="0 0 {width} {out_h_i}" preserveAspectRatio="xMidYMin meet" '
        f'style="display:block;margin:0 auto;">'
    )
    parts.append(
        "<defs><style>"
        "text{font-family:ui-sans-serif,system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial;}"
        ".bp{stroke:rgba(15,23,42,0.38);stroke-width:1.35;}"
        ".bb{stroke:rgba(148,163,184,0.70);stroke-width:1;}"
        ".bA{fill:rgb(145,171,210);}"
        ".bC{fill:rgb(94,153,77);}"
        ".bG{fill:rgb(188,166,205);}"
        ".bU{fill:rgb(245,150,148);}"
        ".bN{fill:rgb(148,163,184);opacity:0.70;}"
        f".btxt{{font-size:{font_px:.2f}px;font-weight:900;fill:rgba(255,255,255,0.96);text-anchor:middle;dominant-baseline:middle;}}"
        "</style></defs>"
    )
    parts.append(f'<rect x="0" y="0" width="{width}" height="{out_h_i}" fill="white" opacity="0"/>')

    # Draw pair lines (shorten endpoints so they don't touch circles)
    shorten = r_draw * 0.75
    for i, j in pair_list:
        x1, y1 = coords[i]
        x2, y2 = coords[j]
        dx = x2 - x1
        dy_ = y2 - y1
        dist = math.hypot(dx, dy_) or 1.0
        ux, uy = dx / dist, dy_ / dist
        sx1, sy1 = x1 + ux * shorten, y1 + uy * shorten
        sx2, sy2 = x2 - ux * shorten, y2 - uy * shorten
        tx1, ty1 = T(sx1, sy1)
        tx2, ty2 = T(sx2, sy2)
        parts.append(
            f'<line class="bp" x1="{tx1:.2f}" y1="{ty1:.2f}" x2="{tx2:.2f}" y2="{ty2:.2f}"/>'
        )
    ...
    for k in range(n):
        x, y = coords[k]
        x, y = T(x, y)
        base = _safe_svg_text(seq[k])
        bclass = "bN"
        if base in {"A", "C", "G", "U"}:
            bclass = f"b{base}"
        parts.append(f'<circle class="bb {bclass}" cx="{x:.2f}" cy="{y:.2f}" r="{r_draw}"/>')
        parts.append(f'<text class="btxt" x="{x:.2f}" y="{y:.2f}">{base}</text>')
...
def predict_one_region(
    *,
    ...
    return {
        ...
        "hairpin_svg": render_hairpin_svg(seq=original_rna, structure=secondary_structure),
