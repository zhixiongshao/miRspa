...
from agent_chat import (  # noqa: E402
    list_providers,
    run_agent_chat,
    verify_api_key,
)
from rna_predict import (  # noqa: E402
    mature_region_from_prediction,
    parse_depths_payload,
    predict_rna,
)
"""
...
def render_hairpin_r2dt_svg(
    *,
    ...
    # R2DT fold_type 允许值示例：full_molecule / insertions_only / all_constraints_enforced
    fold_type = (os.environ.get("R2DT_FOLD_TYPE") or "full_molecule").strip() or "full_molecule"
    # 注意：R2DT 的 constraint 参数通常是“是否使用约束”的开关/枚举，而不是 dot-bracket 本体；
    # dot-bracket 会放在 FASTA 第三行随 sequence 一起提交。
    # 经验：R2DT 这里要求 constraint 参数是字符串 "true"，否则会 400
    constraint_flag = (os.environ.get("R2DT_CONSTRAINT_FLAG") or "true").strip() or "true"
...
def extract_stem_dna_for_region(
    *,
    chrom: str,
    start_1b: int,
    end_1b: int,
    ...
    """返回 (stem_dna, seq_id, rna_sequence)。"""
    chrom_key = normalize_chrom(chrom)
    if start_1b < 1 or end_1b < 1:
        raise ValueError("坐标必须 >= 1")
    if end_1b < start_1b:
    ...
    return dna, seq_id, rna


def predict_rna_region(
    *,
    chrom: str,
    start_1b: int,
    end_1b: int,
    ...
    expected_len = int(end_1b - start_1b + 1)
    reads = parse_depths_payload(depths, expected_len=expected_len)
    dna, seq_id, rna = extract_stem_dna_for_region(
        chrom=chrom,
        start_1b=start_1b,
        end_1b=end_1b,
        strand=strand,
    ...
    try:
        ...
        _structure, mfe_raw = RNA.fold(rna)
    except Exception as e:
        raise ValueError(f"RNA.fold 失败: {e}") from e

    ckpt_rna = Path(
        os.environ.get("CHECKPOINT_RNA", str(args.checkpoint_rna or DEFAULT_RNA_CHECKPOINT))
    )
    if not ckpt_rna.is_file():
        raise FileNotFoundError(
            f"找不到 miRspa-RNA 权重: {ckpt_rna}（请推送到 GitHub 或设置 CHECKPOINT_RNA）"
        )

    out = predict_rna(
        reads=reads,
        mfe_raw=float(mfe_raw),
        mean_cov_divisor=float(mean_cov_divisor),
        checkpoint=ckpt_rna,
    ...
    out.update(
        {
            ...
            "stem_length": expected_len,
            "stem_sequence_rna": rna,
            "reads": reads,
        }
    )
    mature_info = mature_region_from_prediction(
        mature_prob=out.get("mature_prob") or [],
        pos_prob=out.get("pos_prob") or [],
        reads=reads,
        stem_len=expected_len,
        stem_start_1b=int(start_1b),
        stem_end_1b=int(end_1b),
        strand=normalize_strand(strand),
        chrom=normalize_chrom(chrom),
        stem_rna=rna,
    )
    out.update(mature_info)
    if out.get("mature_prob") is not None:
        out["mature_prob_profile"] = list(out["mature_prob"][:expected_len])
    return out


def _agent_predict_dna(args: argparse.Namespace, **kwargs) -> dict:
    return predict_one_region(args=args, **kwargs)


def _agent_predict_rna(args: argparse.Namespace, **kwargs) -> dict:
    depths = kwargs.pop("depths")
    return predict_rna_region(
        depths=depths,
        args=args,
        chrom=kwargs["chrom"],
        start_1b=kwargs["start_1b"],
        end_1b=kwargs["end_1b"],
        strand=kwargs["strand"],
        mean_cov_divisor=kwargs["mean_cov_divisor"],
    )
...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/health":
            ...
            return
        if parsed.path == "/api/health-rna":
            ckpt_rna = Path(
                os.environ.get(
                    "CHECKPOINT_RNA",
                    str(self.server.args.checkpoint_rna or DEFAULT_RNA_CHECKPOINT),
                )
            )
            json_response(
                self,
                ...
                {
                    "ok": True,
                    "checkpoint": str(ckpt_rna),
                    "checkpoint_exists": ckpt_rna.is_file(),
                    "default_mean_cov_divisor": 10000.0,
                },
            )
            return
        if parsed.path == "/api/agent/providers":
            json_response(self, HTTPStatus.OK, list_providers())
            return
        json_response(self, HTTPStatus.NOT_FOUND, {"ok": False, "error": "Not found"})

    def do_POST(self) -> None:
        parsed = urlparse(self.path)
        if parsed.path == "/api/agent/verify":
            try:
        ...
        if parsed.path == "/api/agent/chat":
            ...
            try:
                ...
                def predict_dna(**kw: object) -> dict:
                    return _agent_predict_dna(args, **kw)

                def predict_rna_tool(**kw: object) -> dict:
                    return _agent_predict_rna(args, **kw)
        ...
        if parsed.path == "/api/predict-rna":
            pending = _assets_ready_for_predict()
            if pending is not None:
                json_response(
                    self,
            ...
            try:
                ...
                depths = data.get("depths", data.get("reads"))
                divisor = float(data.get("mean_cov_divisor", 10000.0))
                if not np.isfinite(divisor) or divisor <= 0:
                    raise ValueError("mean_cov_divisor 必须是 > 0 的有限数")
                result = predict_rna_region(
                    chrom=chrom,
                    start_1b=start_1b,
                    end_1b=end_1b,
                    strand=strand,
                    depths=depths,
                    mean_cov_divisor=divisor,
                    args=self.server.args,
                )
                json_response(self, HTTPStatus.OK, result)
            except Exception as e:
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        "--checkpoint-rna",
        type=Path,
        default=Path(os.environ["CHECKPOINT_RNA"])
        if "CHECKPOINT_RNA" in os.environ
        else DEFAULT_RNA_CHECKPOINT,
        help="miRspa-RNA 权重（默认 web/models/miRspa-RNA/best.pt）",
    )
