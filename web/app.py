...
def render_hairpin_svg(
    *,
    ...
    # Rotate to a tilted layout (45°) for better aesthetics.
    if coords:
        ...
        def R(x: float, y: float) -> tuple[float, float]:
            # 45° clockwise rotation around (cx0, cy0)
            # [x']   [ cos  sin][x]
            # [y'] = [-sin  cos][y]
            dx = x - cx0
            dy = y - cy0
            a = 0.7853981633974483  # pi/4
            ca = 0.7071067811865476
            sa = 0.7071067811865476
            rx = dx * ca + dy * sa
            ry = -dx * sa + dy * ca
    ...
    # We estimate the minimal distance in the final (T-transformed) coordinates.
    import math
    pts: list[tuple[float, float]] = [T(*coords[k]) for k in range(n)]
    min_d = float("inf")
    # O(n^2) is fine here (n is typically <= 1000). This avoids rare overlaps after rotation/fit.
    for i in range(n):
        x1, y1 = pts[i]
    ...
    font_px = max(13.0, min(18.0, r_draw * 1.20))
