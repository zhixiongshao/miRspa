...
def render_hairpin_svg(
    *,
    ...
    # Use a larger spacing than the base radius to avoid overlaps after rotation.
    r = 9
    cx = width * 0.50
    top = 88
    dy = max(22, int(r * 2.3))  # ensure circles don't overlap
    stem_gap = 132  # widen stem so paired bases separate more after rotation
    ...
    parts.append(
        ...
        ".bA{fill:rgba(145,171,210,0.92);}"
        ".bC{fill:rgba(94,153,77,0.92);}"
        ".bG{fill:rgba(188,166,205,0.92);}"
        ".bU{fill:rgba(245,150,148,0.92);}"
        ".bN{fill:rgba(148,163,184,0.55);}"
        ".btxt{font-size:11px;font-weight:900;fill:rgba(255,255,255,0.96);text-anchor:middle;dominant-baseline:middle;}"
        "</style></defs>"
    )
    ...
    for k in range(n):
        ...
        if base in {"A", "C", "G", "U"}:
            bclass = f"b{base}"
        parts.append(f'<circle class="bb {bclass}" cx="{x:.2f}" cy="{y:.2f}" r="{r}"/>')
