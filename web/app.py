...
def render_hairpin_svg(
    *,
    ...
    dy = max(28, int(r_layout * 3.0))  # step between stem rungs
    stem_gap = 152  # wider stem so paired bases separate more after rotation
    ...
    # Fit as large as possible without clipping.
    ...
    r_draw = 8.6
    if min_d != float("inf"):
        r_draw = min(r_draw, max(4.8, (min_d * 0.5) - 1.2))
    font_px = max(9.6, min(12.0, r_draw * 1.30))
