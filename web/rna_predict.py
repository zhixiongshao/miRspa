...
def infer_primary_mature_span(
    mature_prob: list[float] | np.ndarray,
    stem_len: int,
    *,
    threshold: float = 0.5,
    min_len: int = 18,
    ...
    """在 stem profile 上选取平均 mature_prob 最高的成熟体窗口。"""
    n = int(stem_len)
    if n <= 0:
        return None
    probs = np.asarray(mature_prob[:n], dtype=np.float64)
    if probs.size == 0 or not np.isfinite(probs).any():
        return None

    best: tuple[int, int] | None = None
    best_score = -1.0
    hi_len = min(int(max_len), n)
    lo_len = min(int(min_len), hi_len)
    for win in range(lo_len, hi_len + 1):
        for i in range(0, n - win + 1):
            seg = probs[i : i + win]
            score = float(seg.mean())
            if score > best_score:
                best_score = score
                best = (i, i + win - 1)
    ...
    if best is None:
        return None
    lo, hi = best
    if float(probs[lo : hi + 1].max()) < float(threshold):
        return None
    return lo, hi
...
def mature_region_from_prediction(
    *,
    ...
    span = infer_primary_mature_span(mature_prob, stem_len, threshold=threshold)
    if span is None:
        return {
            "mature_stem_start_idx": None,
            "mature_stem_end_idx": None,
            "mature_start_1b": None,
            "mature_end_1b": None,
            "mature_sequence_rna": None,
            "mature_prob_mean": None,
        }
    lo, hi = span
    mature_start_1b = int(stem_start_1b) + int(lo)
    mature_end_1b = int(stem_start_1b) + int(hi)
    seq = stem_rna[lo : hi + 1] if stem_rna else ""
    seg = np.asarray(mature_prob[lo : hi + 1], dtype=np.float64)
    return {
        "mature_stem_start_idx": int(lo),
        "mature_stem_end_idx": int(hi),
        "mature_start_1b": mature_start_1b,
        "mature_end_1b": mature_end_1b,
        "mature_sequence_rna": seq,
        "mature_prob_mean": float(seg.mean()) if seg.size else None,
    }
