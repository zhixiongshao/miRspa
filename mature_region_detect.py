...
def _expand_by_reads_plateau(
    reads: np.ndarray | None,
    i0: int,
    i1: int,
    *,
    L: int,
    max_len: int = MAX_MATURE_LEN,
    high_frac: float = 0.90,
    step_frac: float = 0.85,
) -> tuple[int, int]:
    """沿覆盖度平台向两侧扩展：邻位与峰同高，或相对边界缓降则纳入。"""
    if reads is None:
        return i0, i1
    r = np.asarray(reads[:L], dtype=np.float64)
    if r.size == 0 or i1 < i0:
        return i0, i1

    i0, i1 = int(i0), int(i1)
    ref_high = float(r[i0 : i1 + 1].max())
    if ref_high <= 0:
        return i0, i1
    high_thr = ref_high * float(high_frac)

    while i0 > 0:
        if i1 - (i0 - 1) + 1 > max_len:
            break
        left = float(r[i0 - 1])
        if left >= high_thr or left >= float(r[i0]) * step_frac:
            i0 -= 1
        else:
            break

    while i1 < L - 1:
        if (i1 + 1) - i0 + 1 > max_len:
            break
        right = float(r[i1 + 1])
        if right >= high_thr or right >= float(r[i1]) * step_frac:
            i1 += 1
        else:
            break
...
def find_mature_regions(
    ...
    pos: np.ndarray,
    raw_len: int,
    reads: np.ndarray | list[float] | None = None,
) -> list[dict[str, Any]]:
    """
    ...
    if reads is not None:
        rd = np.asarray(reads[:L], dtype=np.float64)
    else:
        rd = None
    ...
    for a0, a1 in runs:
        ...
        a0, a1 = _expand_by_reads_plateau(rd, a0, a1, L=L)
        sub = m[a0 : a1 + 1]
