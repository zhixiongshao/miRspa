    ...
    *,
    ...
    for bi in range(nbins):
        ...
        for mode, out_list in (
            ...
            # When batching (B positions), mfe/seq_len must match batch size if use_mfe=True.
            if use_mfe:
                bsz = int(batch_x.shape[0])
                mfe_b = np.full(bsz, float(mfe_np.reshape(-1)[0]), dtype=np.float32)
                seq_b = np.full(bsz, float(seq_len_np.reshape(-1)[0]), dtype=np.float32)
                probs_m = forward_batch(model, batch_x, True, mfe_b, seq_b, device)
            else:
                probs_m = forward_batch(model, batch_x, False, None, None, device)
